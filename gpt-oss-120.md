Aquí tienes una solución completa. Para poder determinar si el rostro se ha acercado, alejado o movido lateralmente, necesitamos una referencia. La mejor opción es tomar una "imagen base" o "plantilla" cuando el rostro está en una posición neutra y luego comparar la posición actual del rostro con esta referencia.

El código que te proporciono incluye:
1.  **Inicialización:** Configura la cámara y la pantalla LCD.
2.  **Fase de "Captura":** Al inicio, el script espera a que se detecte un rostro. Una vez detectado, toma una "foto" de la posición del centro del rostro y la guarda como referencia. Simultáneamente, el recuadro verde se dibuja con un color diferente (por ejemplo, naranja) para indicar que se está capturando la referencia.
3.  **Fase de "Seguimiento":** Tras capturar la referencia, el sistema entra en modo de seguimiento. Compara la posición del centro del rostro en tiempo real con la posición de referencia.
4.  **Lógica de Comparación:**
    *   **Eje X (Izquierda/Derecha):** Compara la coordenada `x` del centro del rostro. Si `x_actual > x_referencia + margen`, el rostro se ha movido a la derecha. Si `x_actual < x_referencia - margen`, se ha movido a la izquierda.
    *   **Eje Y (Cerca/Alejado):** Compara la coordenada `y` del centro del rostro. *Nota importante:* En la mayoría de las cámaras, `y` aumenta de arriba a abajo. Por lo tanto, si `y_actual < y_referencia - margen`, el rostro se ha acercado (subió hacia arriba), y si `y_actual > y_referencia + margen`, se ha alejado (bajó hacia abajo).
5.  **Dibujado en pantalla:** Dibuja el recuadro verde y anexa una etiqueta de texto que indica el estado (e.g., "IZQUIERDA", "ACERCANDO").

He configurado el script para que se ejecute localmente en el dispositivo K210, sin dependencias externas en tiempo de ejecución.

### Script en MicroPython (K210)
